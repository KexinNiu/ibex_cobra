...
# from funcarve_utils import convert_distance_to_confidence


# data = convert_distance_to_confidence(data, '../data/')
...
if __name__ == "__main__":
    ...
    for i in range(start,int(inter)+start):
        if start != 1:
            pr2ec,ec2pr,predscore,allec2pr = read_cleandf_withscore(input_file,threshold=threshold)
            updateprs = ['1','1']
        print('*'*50)
        ...
        if len(differpredscore.keys()) <= 300:
            for key,val in differpredscore.items():
                print(key,val,sep=':')
        else:
            print('Too many differen predscore only print the first 100')
            c=0
            for key,val in differpredscore.items(): 
                if key in updateprs:
                    print('>>',key,val,sep=':')
