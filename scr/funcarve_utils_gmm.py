    ...
    for r in newreactions:
        ...
        for ec in ecid:
            try:
                ...
                continue
            try:
                pr = max(prd, key=prd.get) # take the pr with MIN val for PREDscore
            except ValueError:
                count+=1
                continue
            try:
                s = min(float(newallpr2ec[pr][ec]) + reward, 1)
                if s >= threshold:
                    updateprs.append(pr)
...
# upper=15,lower=5,maxweight=100,minweight=0.0 
        ...
        # upper=0.8,lower=0.5,maxweight=100,minweight=0.0 
        for rxn in universal.reactions:
            if rxn.id in orig_rxn_ids:
                coefficientDict[rxn.forward_variable] = minweight
                coefficientDict[rxn.reverse_variable] = minweight
                ...
                if rxn.id in weight_dict:
                    if weight_dict[rxn.id] > upper:
                        coefficientDict[rxn.forward_variable] = minweight
                        coefficientDict[rxn.reverse_variable] = minweight
                    elif weight_dict[rxn.id] > lower:
                        coefficientDict[rxn.forward_variable] = (upper - weight_dict[rxn.id])*(maxweight-minweight)/(upper-lower)+minweight
                        coefficientDict[rxn.reverse_variable] = (upper - weight_dict[rxn.id])*(maxweight-minweight)/(upper-lower)+minweight
                else:
                    coefficientDict[rxn.forward_variable] = maxweight
                    coefficientDict[rxn.reverse_variable] = maxweight
        # Create objective, based on pFBA
