...
parser.add_argument('--threshold', default = 0.3, help='do you want to perform the test suite?')
parser.add_argument('--conti',default='no',help='do you want to perform the test suite?')
        ...
        for line in inFile:
            ...
            for item in items:
                if item.startswith('EC:'):
                    ...
                    #         # predscore[pr] = {ecid:dis} 
    print('pr2ec-protein number->',len(list(pr2ec.keys())))   
    return pr2ec,ec2pr,predscore,allec2pr

def clean2seedr(predscore,threshold):
    universal_scoredict={}
    rxns = {}
            ...
            if ec in seedec2r.keys():
                ...
                for rid in rids:
                    ...
                    if rid in universal_scoredict.keys():
                        ...
                        universal_scoredict[rid]=max(score,ori)
...
def update_predscore(newreactions,allec2pr,newpredscore,updateprs,reward,threshold):
    ...
    for r in newreactions:
        ...
        for ec in ecid:
            ...
            try:
                ...
                if float(newpredscore[pr][ec]) >= threshold:
                    updateprs.append(pr)
    ...
    # print('after updataer score',[newpredscore[pr] for pr in updateprs])          
    return newpredscore,updateprs,allec2pr


def get_allec2pr(predscore):
    allec2pr = {}
    for pr, ec_score in predscore.items():
        for ec , score in ec_score.items():
            try:
                allec2pr[ec].update({pr:score})
            except KeyError:
                allec2pr[ec] = {pr:score}
    return allec2pr 
...
if __name__ == "__main__":
    ...
    for i in range(start,int(inter)+1):
        print('*'*50)
        print('Inter:',i)
        print('inter=',inter)
        print('*'*50)
        ...
        if file_type == 5:
            if i ==1:
                ...
                _,_,predscore,allec2pr= read_clean_withscore(clean_file,threshold=threshold)
                ...
                ## load newpredscore and updateprs
                with open(f'{name}_t{thr}_newpredscore_{i-1}.pkl', 'rb') as f:
                    predscore = pickle.load(f)
                # with open(f'{name}_t{thr}_updateprs_{i-1}.pkl', 'rb') as f:
                #     updateprs = pickle.load(f)
                print(f'loaded newpredscore {i-1}')
                universal_scoredict,rxns = clean2seedr(predscore,threshold=threshold)
                allec2pr = get_allec2pr(predscore)
                newpredscore = predscore
                updateprs = []
        ...
        if file_type == 5 or file_type == 6:
            # universal_scoredict,rxns = clean2seedr(predscore)
            # rxns = clean_to_rxns(pr2ec,r2ecf,pr2gene,gene_modelseed,org)
        ...
        #     print('Draft GENRE has', len(rxns), 'reactions\n')
        
        #     # with open('rxns_hits.pickle', 'wb') as f:
        #     #     pickle.dump(rxns, f)
        #     draft_genre = _create_model(rxns, universal, new_id)
        #     draft_genre = _add_names(draft_genre, gene_names)
        else:
        ...
        'cpd00644_e','cpd00654_e','cpd00793_e','cpd00971_e','cpd01012_e','cpd01048_e','cpd03424_e','cpd10515_e','cpd10516_e','cpd11595_e','cpd00020_e'   
            ...
            simulated_media=[
                ...
                # 'cpd',#magnessium sulfate 有镁了
                'cpd00254_e',# Magnesium
                'cpd09400_e',# Sodium sulfate 
                #SrSO4, Na2SiO3(x), 
                'cpd20826_e', #Silica,
                # H3BO3 , NaBr, NH4Cl, NaNO2, RbCl2, LiSO4
                'cpd09695_e', #Sr+
                'cpd09225_e', #Borate
                'cpd00966_e', #Bromide
