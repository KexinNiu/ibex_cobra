#!/bin/bash
#SBATCH --job-name=v3t2UP
#SBATCH -o v3t2UP%A_%a.out
#SBATCH -e v3t2UP%A_%a.err
#SBATCH --mem=150G
...
if [[ $index -ge 0 && $index -lt $num_files ]]; then
    ...
    if [ "$gram" == "none" ]; then
       continue
        ...
        python mainclean_interation_v3.py --input_file $fastadr$file --cleanfile $cleandf${name}_maxsep_df.pkl --block_flage 0 --flux_flage 0 --file_type 7 --reward 0.1 --iter 4 --cpu 8 --gram negative --name $name --media default 
    ...
    echo "python mainclean_interation_v3.py --input_file $fastadr$file --cleanfile $cleandf${name}_maxsep_df.pkl --block_flage 0 --flux_flage 0 --file_type 7 --reward 0.1  --iter 10 --cpu 8 --gram $gram --name $name"
