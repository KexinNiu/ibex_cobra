            ...
            if ec in seedec2r.keys():
                ...
                for rid in rids:
                    ...
                    if rid in universal_scoredict.keys():
                        ...
                        if universal_scoredict[rid] >= 0.8:
...
def update_predscore(newreactions,ec2pr,newpredscore,updateprs,reward,threshold):
    count = 0
...
def _find_reactions(model, reaction_bag, tasks, obj, fraction, max_fraction, step, file_type):
    ...
    thredhold1 = 1e-10
    new_rxn_ids = set([rxn.id for rxn in reaction_bag.reactions if abs(solution.fluxes[rxn.id]) > thredhold1]).difference(orig_rxn_ids)
    print('thredhold:',thredhold1)
    stdout.write('\r[-----------------------------------------]\n')
    ...
    for i in range(1,int(inter)+1):
        ...
        if file_type == 5:
            if i ==1:
                ...
                updateprs = updateprs
                universal_scoredict,rxns = clean2seedr(newpredscore)
        ...
        if gapfill == 'yes':
            ...
            newpredscore, updateprs = update_predscore(new_reactions,ec2pr,newpredscore,updateprs,reward,threshold)
            ...
            if file_type != 3:
                ...
                newpredscore,updateprs = update_predscore(media_reactions,ec2pr,newpredscore,updateprs,reward,threshold)
                print('aft update media new_reactions',media_reactions)
                print('updateprs:',updateprs)
        ...
        ## save the dictionary
        with open(f'{name}_newpredscore_{i}.pkl', 'wb') as f:
            pickle.dump(newpredscore, f)
        print(f'saved newpredscore {i}')
        with open(f'{name}_updateprs_{i}.pkl', 'wb') as f:
