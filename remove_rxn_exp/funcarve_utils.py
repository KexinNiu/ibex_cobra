...
def _find_reactions(model, reaction_bag, tasks, obj, fraction, max_fraction, step, file_type):
    ...
    with reaction_bag as universal: #set the reaction bag as the universal reaction databse
        ...
        for rid in remove_rxns:
            try:
                universal.reacitons.get_by_id(rid).remove_from_model()
            except:
                continue
        ...
        if method == 1:
            ...
            for rxn in universal.reactions:
                if rxn.id in orig_rxn_ids:
                    ...
                    if rxn.id in weight_dict:
                        ...
                        coefficientDict[rxn.forward_variable] = 50
                        coefficientDict[rxn.reverse_variable] = 50
        elif method == 3:
            for rxn in universal.reactions:
                if rxn.id in orig_rxn_ids:
                    coefficientDict[rxn.forward_variable] = 0.0
                    ...
                    if rxn.id in weight_dict:
                        coefficientDict[rxn.forward_variable] = abs(1-weight_dict[rxn.id])
                        coefficientDict[rxn.reverse_variable] = abs(1-weight_dict[rxn.id])
                    else:
                        coefficientDict[rxn.forward_variable] = 20
                        coefficientDict[rxn.reverse_variable] = 20
        elif method == 4:
            ...
            for rxn in universal.reactions:
                if rxn.id in orig_rxn_ids:
                    ...
                    if rxn.id in weight_dict:
                        coefficientDict[rxn.forward_variable] = max_weight *(weight_dict[rxn.id]-min_weight)/(max_weight-min_weight)
                        coefficientDict[rxn.reverse_variable] = max_weight *(weight_dict[rxn.id]-min_weight)/(max_weight-min_weight)
                    else:
                        coefficientDict[rxn.forward_variable] = max_weight * 0.5
                        coefficientDict[rxn.reverse_variable] = max_weight * 0.5
        elif method == 5:
            max_weight = max(weight_dict.values())
            min_weight = min(weight_dict.values())
            for rxn in universal.reactions:
                if rxn.id in orig_rxn_ids:
                    ...
                    if rxn.id in weight_dict:
                        ...
                        coefficientDict[rxn.forward_variable] = max_weight * 0.8
                        coefficientDict[rxn.reverse_variable] = max_weight * 0.8
        elif method == 6:
            max_weight = max(weight_dict.values())
            for rxn in universal.reactions:
                if rxn.id in orig_rxn_ids:
                    ...
                    if rxn.id in weight_dict:
                        if weight_dict[rxn.id] > 10:
                            coefficientDict[rxn.forward_variable] = 1
                            coefficientDict[rxn.reverse_variable] = 1
                        else:
                            coefficientDict[rxn.forward_variable] = weight_dict[rxn.id] -5
                            coefficientDict[rxn.reverse_variable] = weight_dict[rxn.id] -5
                    else:
                        coefficientDict[rxn.forward_variable] = max_weight
                        coefficientDict[rxn.reverse_variable] = max_weight
        elif method == 7:
            for rxn in universal.reactions:
                if rxn.id in orig_rxn_ids:
                    coefficientDict[rxn.forward_variable] = 0.0
                    ...
                    if rxn.id in weight_dict:
                        if weight_dict[rxn.id] > 15:
                            coefficientDict[rxn.forward_variable] = 15
                            coefficientDict[rxn.reverse_variable] = 15
                        else:
                            coefficientDict[rxn.forward_variable] = 15-weight_dict[rxn.id]
                            coefficientDict[rxn.reverse_variable] = 15-weight_dict[rxn.id]
                    else:
                        coefficientDict[rxn.forward_variable] = 15
                        coefficientDict[rxn.reverse_variable] = 15
        elif method == 8:
            ...
            for rxn in universal.reactions:
                if rxn.id in orig_rxn_ids:
                    ...
                    if rxn.id in weight_dict:
                        coefficientDict[rxn.forward_variable] = (weight_dict[rxn.id]-min_weight)/(max_weight-min_weight)
                        coefficientDict[rxn.reverse_variable] = (weight_dict[rxn.id]-min_weight)/(max_weight-min_weight)
                    else:
                        coefficientDict[rxn.forward_variable] = 1
                        coefficientDict[rxn.reverse_variable] = 1
        elif method == 11:
...
def _gapfill_model(model, universal, new_rxn_ids, obj, step):
    ...
    # print('model.metabolites:',len(model.metabolites),list(model.metabolites)[:10])
    for cpd in model.metabolites:
        # print('cpd:',cpd,cpd.compartment)
        if cpd.compartment != 'extracellular':
            continue
        elif cpd.compartment == 'extracellular' or cpd.compartment == 'e':
            cpd.compartment = 'e'
            try:
