...
from funcarve_utils import read_fasta, _checkModel, read_cleandf_withscore, _find_reactions, _gapfill_model, _add_annotation, weighted_find_reactions, print_model_info,read_clean_withscore,clean2biggr,_set_base_inputs
import mergem
...
pr2ec,ec2pr,predscore,allec2pr = read_cleandf_withscore('/ibex/user/niuk0a/CLEAN/app/results/inputs/CP000148.1_t4_maxsep_df.pkl',threshold=8)
...
universal_scoredict = clean2biggr(predscore,8)
...
cobra.io.write_sbml_model(final_genre, f'/ibex/user/niuk0a/funcarve/cobra/remove_rxn_exp/iAF987_{str(args.seednum)}_{str(args.prec)}_{args.type}_gf.xml')
...
# pr2ec , predscore = read_clean_withscore('/ibex/user/niuk0a/funcarve/cobra/aaseq_CP000148.1_maxsep.csv')
pr2ec,ec2pr,predscore,allec2pr = read_cleandf_withscore('/ibex/user/niuk0a/CLEAN/app/results/inputs/CP000148.1_t4_maxsep_df.pkl',threshold=8)
universal_scoredict,rxns, r2maxecp = clean2biggr(predscore,8) # r2maxecp[rid]=[pr,ec,score]
print('Identifying new metabolism...')
...
new_reactions_ids ,coefficientDict,new_rxn_flux = weighted_find_reactions(model, universal, metabolic_tasks, universal_obj, min_frac, max_frac, 1, file_type,universal_scoredict)
if file_type != 3:
    print('Identifying new metabolism (Step 2 of 2)...')
    ...
    media_reactions ,coefficientDict,new_rxn_flux = weighted_find_reactions(filled_genre, universal, metabolic_tasks, universal_obj, min_frac, max_frac, 2, file_type,universal_scoredict)
