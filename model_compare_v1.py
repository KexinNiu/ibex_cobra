...
def create_model_randomrm(model,universal,prec,seednum,input_id):
    ...
    for r in tmpm.reactions:
        try:
            test = universal.reactions.get_by_id(r.id)
        except:
...
def gapfill_draftmodel(draftmodel,universal,gftype,cleanfile=None):
    ...
    file_type = 1 # 3 for sbml draft model
    ...
    # universal_obj = bigg_obj(universal)
    universal_obj = bigg_obj(model)
    print('universal_obj:',universal_obj)
    universal.add_reactions([draftmodel.reactions.get_by_id(universal_obj)])
    if gftype == 'gf':
        ...
        print('GF new reactions:',len(new_reactions),'|', 'GF new reactions:',new_reactions)
        ...
        final_genre = _gapfill_model(filled_genre, universal, media_reactions, universal_obj, 2)
        final_genre = _add_annotation(filled_genre, universal_obj)
        print('GF new reactions:',len(new_reactions_ids),'|', 'GF new reactions:',new_reactions_ids)
        # print('GF media reactions:',len(media_reactions),'\n', 'GF media reactions:',media_reactions)
...
if __name__ == '__main__':
    ...
    # save for carveme 
    cobra.io.write_sbml_model(model, f'/ibex/user/niuk0a/funcarve/cobra/iAF987_{str(args.seednum)}_{str(args.prec)}_rm.xml')
